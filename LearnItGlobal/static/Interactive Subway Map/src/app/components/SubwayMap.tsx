...
export function SubwayMap() {
  ...
  return (
    <div className="w-full min-h-screen flex flex-col items-center justify-center p-8 bg-gradient-to-br from-purple-50 to-purple-100" style={{ backgroundColor: '#f3e8f7' }}>
      <div className="mb-8" style={{padding: '20px'}}>
        <h1 className="text-4xl font-bold text-black mb-2">North-East Line</h1>
      </div>

      <div className="flex gap-8 w-full max-w-7xl" style={{padding: '20px'}}>
        {/* Subway Map */}
        <div className="flex-1 bg-white rounded-2xl shadow-2xl p-8">
          <svg
            viewBox={`0 0 200 100}`}
            className="w-full h-auto"
            style={{ height: '504px', width: '261pxx' }}
          >
            ...
            {stations.map((station) => {
              ...
              return (
                <g key={station.id}>
                  ...
                  <text
                    x={station.id < 9 ? station.position.x - 100 : (station.id == 9 ? station.position.x - 30 : station.position.x + 15)}
                    y={station.id == 9 ? station.position.y + 20 : station.position.y + 5}
                    fontSize="13"
